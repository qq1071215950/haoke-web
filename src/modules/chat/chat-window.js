...
/*  componentDidMount = () => {
  ...
  componentDidMount = () => {
    let {to_user, from_user} = this.props.chatInfo;
    axios.get('http://127.0.0.1:8888/message', {params: {toId: to_user, fromId: from_user}}).then(data => {
      this.setState({
        infos: data,
        isLoading: true,
        fromId: from_user,
  ...
  render() {
    ...
    if (this.state.isLoading) {
      ...
      infoList = this.state.infos.map(item => {
        return (<li key={item.id} className={currentUser === item.from.id ? 'chat-info-right' : 'chat-info-left'}>
          <span>{item.msg}</span></li>)
      })
    }
    return (
