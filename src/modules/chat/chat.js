...
class Chat extends React.Component {
  ...
  componentDidMount = () => {
    axios.get('http://127.0.0.1:8888/user?fromId=1001').then((data) => {
      this.setState({
        list: data,
        isLoading: true
      })
  ...
  render() {
    ...
    if (isLoading) {
      list = this.state.list.map(item => {
        return (<li key={item.id} onClick={(e) => this.toChat(e, {item})}>
          <div className="avarter"><img src={item.avatar} alt="avarter"/> <span className="name">{item.username}</span>
            <span className="info">{item.chat_msg}</span> <span className="time">{item.ctime}</span></div>
        </li>)
      })
    }
    return (
